...
csv <- read_csv("csvplantas.csv", locale = locale(encoding = "ISO-8859-1"), skip = 3)
...
#Evaluamos la Z-score y creamos una nueva columna evaluacion donde indicamos el resultado de la evaluación
...
ft3<-flextable(as.data.frame(c("ESTUDIO DE RESULTADOS GLOBALES")))
ft3<-ft3 %>% autofit() %>% fit_to_width(7.5)
ft3<-bold(ft3, i = NULL, j = NULL, bold = TRUE, part = "body")
ft3<-delete_part(ft3, part = "header")
cat(knit_print(ft3))

tablatotal <- data.frame(
  "Valor" = c("Número total de análisis","Número total de anáisis con un *","Número total de análisis con dos **","Número total de análisis con un/dos */**"), 
  "Número" = c(nrow(csv),planta2,planta1,planta1+planta2), 
  "Porcentaje" = c(100,round(((planta2*100)/nrow(csv)),0),round(((planta1*100)/nrow(csv)),0),round((((planta2+planta1)*100)/nrow(csv)),0))
  )


ft2<-flextable(tablatotal)
ft2<-ft2 %>% autofit() %>% fit_to_width(7.5)
cat(knit_print(ft2))
...
## Informe generado automáticamente utilizando el software RStudio, el archivo de R Markdown WEPAL_AUTOMATICO_PLANTAS.Rmd y el archivo de datos obtenido de www.wepal.nl AÑO-PERIODO-IPE-XGCALAFIGA.csv del trimestre correspondiente.
